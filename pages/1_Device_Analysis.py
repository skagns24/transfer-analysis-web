...
# CSS 주입 (멀티페이지 디자인 연동)
st.markdown("""
<style>div.stButton > button {border-radius: 8px; box-shadow: 2px 2px 5px rgba(0,0,0,0.3); transition: all 0.3s ease;} div.stButton > button:hover {transform: translateY(-2px); box-shadow: 2px 5px 10px rgba(0,0,0,0.5);}</style>
...
# =====================================================================
    ...
    if files_to_process_tlm:
        col1, col2 = st.columns([8, 2])
        with col1:
            st.success(f"현재 {len(files_to_process_tlm)}개의 TLM 파일이 분석 중입니다.")
        with col2:
...
# =====================================================================
    ...
    with col_input:
        ...
        vg = st.slider("Gate Bias (Vg)", -5.0, 2.0, 0.0, 0.1)
    ...
    with col_plot:
        c1, c2 = st.columns(2)
        c1.info(f"**Threshold Voltage (Vth)**\n\n### {Vth:.2f} V")
        c2.success(f"**2DEG Density (ns) @ Vg={vg}V**\n\n### {ns_cm2:.2e} cm-2")
        
        fig_band = go.Figure()
        fig_band.add_trace(go.Scatter(x=x, y=Ec, mode='lines', name='Conduction Band (Ec)', line=dict(color='blue', width=3)))
        fig_band.add_trace(go.Scatter(x=x, y=Ev, mode='lines', name='Valence Band (Ev)', line=dict(color='red', width=3)))
        fig_band.add_trace(go.Scatter(x=x, y=Ef, mode='lines', name='Fermi Level (Ef)', line=dict(color='black', width=2, dash='dash')))
    ...
    with st.expander("🔍 상세 물리 파라미터 확인 (Material & Polarization Data)"):
        st.write(f"- **AlGaN Bandgap (Eg)**: {algan['Eg']:.2f} eV")
        st.write(f"- **Conduction Band Offset (dEc)**: {dE_c:.2f} eV")
        st.write(f"- **Spontaneous Polarization (Psp)**: {algan['P_total'] - GaN['Psp']:.4f} C/m2")
